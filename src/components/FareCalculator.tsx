...
import { 
  ...
  Receipt, ArrowRight, MapPin, Clock
} from 'lucide-react';
...
export default function FareCalculator({ fareConfig, onApplyForBooking }: FareCalculatorProps) {
  ...
  // Live dynamic calculation matching CMS fare variables
  const calculations = useMemo(() => {
    const baseFare = fareConfig.baseFare;
    const distanceCost = distance * fareConfig.perKmFare;
    
    let distanceFare = baseFare + distanceCost;
    let isMinApplied = false;

    // Apply minimum fee rule dynamically
    if (distance <= fareConfig.minFareDistanceLimit) {
      distanceFare = fareConfig.minFare;
      isMinApplied = true;
    }

    // Add extra surcharges matching CMS configs
    let optionFare = 0;
    if (options.includes('petcare')) optionFare += fareConfig.petCareSurcharge;
    if (options.includes('waypoint')) optionFare += fareConfig.viaSurcharge;
    if (options.includes('roundtrip_wait')) optionFare += fareConfig.waitingFarePer10Min;
    if (options.includes('night')) optionFare += fareConfig.nightSurcharge;
    if (options.includes('outer')) optionFare += fareConfig.outOfSeoulSurcharge;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12 animate-fadeIn" id="fare-calculator-view">
      ...
      <section className="bg-gradient-to-br from-brand-green/5 to-emerald-50 rounded-3xl border border-brand-green/10 p-6 md:p-8" id="official-fare-sheet-overview">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-gray-100">
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-500">manspet taxi의 투명하고 정직한 정찰제 요금표입니다.</p>
          </div>
            ...
            <div className="space-y-2 text-xs">
              <div className="bg-brand-green/5 p-3 rounded-xl border border-brand-green/5 text-center">
                ...
                <p className="font-extrabold text-gray-900 mt-1">
                  기본요금 {fareConfig.baseFare.toLocaleString()}원 + 1km당 {fareConfig.perKmFare.toLocaleString()}원
                </p>
              </div>
              <p className="text-[11px] text-gray-650 leading-relaxed font-semibold">
                <strong>💡 요금 예시 (10km 운행 시):</strong><br />
                기본요금 {fareConfig.baseFare.toLocaleString()}원 + 10km 요금 {(10 * fareConfig.perKmFare).toLocaleString()}원 = <strong className="text-brand-green text-xs">총 {(fareConfig.baseFare + 10 * fareConfig.perKmFare).toLocaleString()}원</strong>
              </p>
            ...
            <div className="space-y-2 text-xs">
              <div className="bg-amber-50/70 p-3 rounded-xl border border-amber-100 text-center">
                ...
                <p className="font-black text-gray-900 mt-1">
                  {fareConfig.minFareDistanceLimit}km 이하 운행 시 최저 요금 {fareConfig.minFare.toLocaleString()}원
                </p>
              </div>
              <p className="text-[11px] text-gray-650 leading-relaxed font-normal">
                {fareConfig.minFareDistanceLimit}km 이하의 가까운 단거리 통원이나 이동 시에도 숙련 차량 매칭을 위해 <strong>일률 최저 요금제 {fareConfig.minFare.toLocaleString()}원</strong>이 정직하게 적용됩니다.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100 space-y-3 shadow-2xs">
            ...
            <div className="space-y-2 text-[11px] text-gray-700 font-medium">
              <div className="flex justify-between items-center border-b border-gray-50 pb-1.5">
                <span>펫케어 서비스 (단독케어/진료/픽업 등)</span>
                <span className="font-black text-brand-green shrink-0">+{fareConfig.petCareSurcharge.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between items-center border-b border-gray-50 pb-1.5">
                <span>경로 외 경유 시 (중간 목적지 경유)</span>
                <span className="font-black text-brand-green shrink-0">+{fareConfig.viaSurcharge.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between items-center border-b border-gray-50 pb-1.5">
                <span>왕복 운행 대기 시 (30분 초과)</span>
                <span className="text-gray-400 text-[10px] ml-1">10분당</span>
                <span className="font-black text-brand-green shrink-0">+{fareConfig.waitingFarePer10Min.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between items-center border-b border-gray-50 pb-1.5">
                <span>심야/조조 할증 (09~21시 외 시간)</span>
                <span className="font-black text-brand-green shrink-0">+{fareConfig.nightSurcharge.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between items-center">
                <span>경기 외곽 지역 출발/도착</span>
                <span className="font-black text-brand-green shrink-0">+{fareConfig.outOfSeoulSurcharge.toLocaleString()}원</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100 space-y-3 shadow-2xs">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="bg-amber-50/70 rounded-xl p-2.5 border border-amber-100/50 text-[10.5px] text-amber-950 leading-normal font-semibold">
                ⚠️ <strong className="text-rose-600 font-bold">7월·8월·9월 성수기 시즌</strong>에는 타 승객 예약 선점을 위해 <strong>예약하신 날부터 환불 불가</strong>이오니 이 점 너른 양해 부탁드립니다.
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start pt-4" id="estimator-section">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-8" id="calculator-fields">
          ...
          <div className="space-y-3">
            ...
            {distance <= fareConfig.minFareDistanceLimit ? (
              <div className="bg-amber-50 border border-amber-100 text-amber-800 rounded-xl p-3 flex items-start gap-2 text-xs font-semibold">
                <span>🌟 {fareConfig.minFareDistanceLimit}km 이하의 단거리 운행이므로 <strong>최저 요금제 {fareConfig.minFare.toLocaleString()}원</strong>이 자동 적용됩니다.</span>
              </div>
            ) : (
              <div className="bg-brand-green-light/50 border border-brand-green-light text-brand-green rounded-xl p-3 text-xs font-semibold animate-fadeIn">
                <span>기본운행료: {fareConfig.baseFare.toLocaleString()}원 + 주행거리 요금 ({distance}km x {fareConfig.perKmFare.toLocaleString()}원 = {calculations.rawDistanceCost.toLocaleString()}원)</span>
              </div>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                { 
                  ...
                  price: `+${fareConfig.petCareSurcharge.toLocaleString()}원` 
                },
                { 
                  id: 'waypoint', 
                  label: '경로 외 경유지 추가', 
                  desc: '목적지 가기 전 경유지 지정 운행', 
                  price: `+${fareConfig.viaSurcharge.toLocaleString()}원` 
                },
                { 
                  id: 'roundtrip_wait', 
                  label: '왕복 운행 대기', 
                  desc: '30분 초과 시 10분당 추가요금', 
                  price: `+${fareConfig.waitingFarePer10Min.toLocaleString()}원` 
                },
                { 
                  id: 'night', 
                  label: '야간 할증 요금', 
                  desc: '09:00 ~ 21:00 외 시간대 운행', 
                  price: `+${fareConfig.nightSurcharge.toLocaleString()}원` 
                },
                { 
                  id: 'outer', 
                  label: '서울 외곽 지역 운행', 
                  desc: '인천, 파주, 분당, 의정부 등지 하차', 
                  price: `+${fareConfig.outOfSeoulSurcharge.toLocaleString()}원` 
                }
              ].map((o) => (
                <button
                  key={o.id}
            ...
            <div className="space-y-3.5 text-xs font-semibold" id="receipt-items">
              ...
              {calculations.isMinApplied && (
                <div className="flex justify-between items-center text-brand-yellow text-[10px]">
                  <span>└ {fareConfig.minFareDistanceLimit}km 이하 단거리 최저요금제 보장</span>
                  <span>적용완료</span>
                </div>
              )}
                ...
                <div className="text-[10px] text-green-250 font-normal pl-2 space-y-1">
                  {options.map(o => (
                    <div key={o} className="flex justify-between">
                      ...
                      <span>{
                        o === 'roundtrip_wait' 
                          ? `+${fareConfig.waitingFarePer10Min.toLocaleString()}원` 
                          : o === 'petcare' 
                            ? `+${fareConfig.petCareSurcharge.toLocaleString()}원`
                            : o === 'waypoint' 
                              ? `+${fareConfig.viaSurcharge.toLocaleString()}원`
                              : o === 'night' 
                                ? `+${fareConfig.nightSurcharge.toLocaleString()}원`
                                : `+${fareConfig.outOfSeoulSurcharge.toLocaleString()}원`
                      }</span>
                    </div>
                  ))}
                </div>
      ...
      {showAppliedToast && (
        <div className="fixed bottom-6 right-6 bg-gray-900 border border-gray-800 text-white rounded-2xl p-4 shadow-2xl z-50 flex items-center gap-2.5 animate-slideUp">
          <div className="p-1.5 bg-brand-green text-white rounded-full font-bold">
            <CheckCircle2 className="w-4 h-4" />
          </div>
          <div>
            <p className="text-xs font-extrabold font-sans">안심 요금 정보 복사 및 이동 완료!</p>
            <p className="text-[10px] text-gray-400 mt-0.5">클립보드에 작성된 견적서를 네이버 톡톡 상담창에 붙여넣기 하시면 즉시 예약을 돕습니다.</p>
          </div>
        </div>
      )}
    </div>
